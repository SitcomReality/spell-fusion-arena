    ...
    for (let i = 0; i < this.totalSlots; i++) {
      ...
      if (i < this.unlockedSlots) {
        ...
        if (this.selectedElements[i]) {
          // Build a content container and insert a VisualPreview tile (fallback to color block)
          const content = document.createElement('div');
          content.className = 'fusion-slot-content';
          const title = document.createElement('span');
          title.textContent = this.selectedElements[i].name;
          ...
          // Try to create VisualPreview and insert behind text/controls
          try {
            const previewData = {
              color: this.selectedElements[i].color,
              secondaryColor: this.selectedElements[i].secondaryColor,
              accentColor: this.selectedElements[i].accentColor || this.selectedElements[i].secondaryColor,
              properties: this.selectedElements[i].propertyGenes,
              visualEffects: this.selectedElements[i].visualEffects
            };
            ...
            // Insert preview as first child so controls overlay it
            content.insertBefore(previewEl, content.firstChild);
          } catch (e) {
            const fallback = document.createElement('div');
            fallback.className = 'fusion-slot-fallback-color';
            fallback.style.position = 'absolute';
            fallback.style.inset = '6px';
            fallback.style.borderRadius = '6px';
            fallback.style.background = `rgb(${this.selectedElements[i].color.r}, ${this.selectedElements[i].color.g}, ${this.selectedElements[i].color.b})`;
